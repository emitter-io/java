...
public class FutureConnection {
    ...
    public Future<byte[]> subscribe(final Topic topic){
    ...
    public Future<Void> publish(final String key, final String channel, final byte[] payload) {
    	String topic = key == null 
    			? channel
    			: key + "/" + channel;
    	
        return publish(utf8(topic), new Buffer(payload), QoS.AT_MOST_ONCE, false);
    }

    Future<Void> publish(final UTF8Buffer topic, final Buffer payload,  final QoS qos, final boolean retain) {
        final Promise<Void> future = new Promise<Void>();
        next.getDispatchQueue().execute(new Task() {
            public void run() {
