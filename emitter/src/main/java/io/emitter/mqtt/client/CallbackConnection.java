...
import io.emitter.mqtt.codec.MQTTProtocolCodec;
import io.emitter.mqtt.codec.MessageSupport.Acked;
import io.emitter.mqtt.codec.PINGREQ;
import io.emitter.mqtt.codec.PINGRESP;
import io.emitter.mqtt.codec.PUBACK;
import io.emitter.mqtt.codec.PUBCOMP;
import io.emitter.mqtt.codec.PUBLISH;
import io.emitter.mqtt.codec.PUBREC;
import io.emitter.mqtt.codec.PUBREL;
import io.emitter.mqtt.codec.SUBACK;
import io.emitter.mqtt.codec.SUBSCRIBE;
import io.emitter.mqtt.codec.UNSUBACK;
import io.emitter.mqtt.codec.UNSUBSCRIBE;
...
public class CallbackConnection {
    ...
    public void publish(String key, String channel, byte[] payload, Callback<Void> cb) {
        ...
        publish(utf8(topic), new Buffer(payload), QoS.AT_MOST_ONCE, false, cb);
    }

    void publish(UTF8Buffer topic, Buffer payload, QoS qos, boolean retain, Callback<Void> cb) {
        queue.assertExecuting();
        if( disconnected ) {
    ...
    public void subscribe(final Topic topic, Callback<byte[]> cb){
