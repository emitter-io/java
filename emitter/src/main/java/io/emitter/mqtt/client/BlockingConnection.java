...
public class BlockingConnection {
    ...
    void publish(final UTF8Buffer topic, final Buffer payload, final QoS qos, final boolean retain) throws Exception {
    ...
    public void publish(final String key, final String channel, final byte[] message) throws Exception {
    	String topic = key == null 
    			? channel
    			: key + "/" + channel;
    	
        publish(utf8(topic), new Buffer(message), QoS.AT_MOST_ONCE, false);
    }
